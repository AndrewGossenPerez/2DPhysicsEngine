...
std::vector<Vec2> generateRegularPolygon(int sides, float radius){
    ...
    // Optionally rotate so one vertex points up
...
float computeRegularPolygonInertia(int sides, float mass, float radius){ 
    if (sides < 3 || mass <= 0.0f) return 0.0f; // invalid or static
...
RigidBody::RigidBody(int n,float r,float mass) : sides(n), radius(r), mass(mass) {
    ...
    inverseMass=computeInverseMass(mass,isStatic); // isStatic is initialsied at false still as inverse mass is initialised at zero
}